...
public class Fiddling extends PApplet {
  ...
  public static void main(String args[]) {
    ...
    PeasyCam cam;
    Controls controls;
    float thetaDelta = (float) 1.64f;
    float theta = (float) 1.64f;
    int blocksize = 5;
    ...
    public void setup() {
        this.size(1024, 764, P3D   );
        
        frameRate(25);
        
        if (NUM_EIMAGES == 6) {
            thetaDelta = (float) 3.35f;
            theta = (float) 5.27f;
        }
        // println(gl);
    ...
    public void draw() {
        ...
        if (userList.size() > 0) {
            background(19);
            lightFalloff((float) 0.8f, (float) 0.2f, 0);
    ...
    public void keyPressed() {
        if (key == 'p') {
            ...
            this.scale((float) (scale * .75f));
        else if (key == 'b')
            this.scale((float) (scale * 1.25f));
